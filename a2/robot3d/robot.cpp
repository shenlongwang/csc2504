...
GLfloat light_direction[] = { 25.0, 25.0, 30.0, 0.0 };
...
int maxValidKeyframe   = 5;		// index of max VALID keyframe (in keyframe list)
...
const float DUMP_SEC_PER_FRAME = 2.0 / DUMP_FRAME_PER_SEC;
...
void initGlui()
{
	...
	glui_panel = glui_joints->add_panel("Scale");

	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "scale left:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::SCALE_LEFT));
	glui_spinner->set_float_limits(SCALE_MIN, SCALE_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);

	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "scale right:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::SCALE_RIGHT));
	glui_spinner->set_float_limits(SCALE_MIN, SCALE_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
	...
	// Create controls to specify light position 

	glui_spinner = glui_render->add_spinner_to_panel(glui_panel, "light position x :", GLUI_SPINNER_FLOAT, &light_direction[0]);
	glui_spinner->set_float_limits(LIGHT_POS_MIN, LIGHT_POS_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);

	glui_spinner = glui_render->add_spinner_to_panel(glui_panel, "light position y:", GLUI_SPINNER_FLOAT, &light_direction[1]);
	glui_spinner->set_float_limits(LIGHT_POS_MIN, LIGHT_POS_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);

	glui_spinner = glui_render->add_spinner_to_panel(glui_panel, "light position z :", GLUI_SPINNER_FLOAT, &light_direction[2]);
	glui_spinner->set_float_limits(LIGHT_POS_MIN, LIGHT_POS_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
...
void display(void)
{
	...
	// SAMPLE CODE **********
	//
    const float body_height = 1.0;
    const float body_width = 0.6;
    const float arm_width = 0.2;
    const float body_depth = 0.2;
    ...
    glPushMatrix();
      ...
      //   glMaterialfv( GL_FRONT, GL_SHININESS, shininess);
        ...
        // draw lower body
        glPushMatrix();
            glScalef(1/body_width, 1/body_height, 1/body_depth);
            glTranslatef(0.0, -body_height-arm_width, 0.0);
            glScalef(body_height, arm_width, body_depth);
            ...
            // draw left leg 
            glPushMatrix();
                glScalef(1/body_height, 1/arm_width, 1/body_depth);
                glTranslatef(-body_height+arm_width, 0.0, 0.0);
                ...
                glTranslatef(0.0, -body_height+arm_width, 0.0);
                glScalef(arm_width, body_height, body_depth);
        	    glColor3f(0.5, 0.5, 0.5);
                getRenderStyle();
                drawCube();
            glPopMatrix(); // draw circle 
            glPushMatrix();
                glScalef(1/body_height, 1/arm_width, 1/body_depth);
                glTranslatef(-body_height+arm_width, 0.0, 0.0);
                glScalef(circle_radius, circle_radius, body_depth);
        ...
        glPushMatrix();
            glScalef(1/body_width, 1/body_height, 1/body_depth);
            glTranslatef(-body_width+arm_width, arm_width, 0.0);
            ...
            glPushMatrix();
                ...
                glTranslatef(-joint_ui_data->getDOF(Keyframe::SCALE_LEFT)+arm_width, 0.0, 0.0);
                glScalef(joint_ui_data->getDOF(Keyframe::SCALE_LEFT), arm_width, body_depth);
        	    glColor3f(0.5, 0.5, 0.5);
                getRenderStyle();
                drawCube();
            glPopMatrix();
        ...
        glPushMatrix();
            glScalef(1/body_width, 1/body_height, 1/body_depth);
            glTranslatef(-body_width+arm_width, arm_width, 0.0);
            glScalef(circle_radius, circle_radius, 1.1*body_depth);
            glColor3f(0.0, 0.0, 0.0);
            drawCylinder();
        glPopMatrix();
        ...
        glPushMatrix();
            ...
            glPushMatrix();
                ...
                glTranslatef(joint_ui_data->getDOF(Keyframe::SCALE_RIGHT)-arm_width, 0.0, 0.0);
                glScalef(joint_ui_data->getDOF(Keyframe::SCALE_RIGHT), arm_width, body_depth);
        	    glColor3f(0.5, 0.5, 0.5);
                getRenderStyle();
                drawCube();
            glPopMatrix();
        ...
        glPushMatrix();
            glScalef(1/body_width, 1/body_height, 1/body_depth);
            glTranslatef(0.0, body_height-arm_width, 0.0);
            glTranslatef(0.0, body_width-arm_width, 0.0);
            glScalef(arm_width, body_width, 0.25*body_depth);
		    glColor3f(0.5, 0.5, 0.5);
            getRenderStyle();
            drawCube();
            // draw head  
            glPushMatrix();
                glScalef(1/arm_width, 1/body_width, 4/body_depth);
                glTranslatef(0.0, body_height, 0.0);
                glRotatef(joint_ui_data->getDOF(Keyframe::HEAD_ROLL), 1.0, 0.0, 0.0);
                glRotatef(joint_ui_data->getDOF(Keyframe::HEAD_YAW), 0.0, 1.0, 0.0);
                glRotatef(joint_ui_data->getDOF(Keyframe::HEAD_PITCH), 0.0, 0.0, 1.0);
                glRotatef(0.0, 0.0, 0.0, 1.0);
                glScalef(body_height, body_width, body_depth);
        	    glColor3f(0.5, 0.5, 0.5);
                getRenderStyle();
                drawCube();
                // draw eye  
                glPushMatrix();
                    glScalef(1/body_height, 1/body_width, 1/body_depth);
                    glTranslatef(-body_width, 0.0, 0.0);
                    glRotatef(0.0, 0.0, 0.0, 1.0);
                    glScalef(0.3, 0.3, 1.2*body_depth);
            	    glColor3f(0.0, 0.0, 0.0);
                    drawCylinder();
                    glScalef(0.8, 0.8, 1.2*body_depth);
            	    glColor3f(0.0, 0.0, 0.0);
                    drawCylinder();
                glPopMatrix();
                // draw eye  
        ...
        glPushMatrix();
            glScalef(1/body_width, 1/body_height, 1/body_depth);
            glTranslatef(0.0, body_height-arm_width, 0.0);
            glScalef(circle_radius, circle_radius, 1.2*body_depth);
            glColor3f(0.0, 0.0, 0.0);
            drawCylinder();
        glPopMatrix();
	glPopMatrix();
...
const float THETA = PI/180;
void drawCircle(int radius)
{
    glBegin(GL_LINE_LOOP);
    for (int i = 0; i < 360; i++)
    {
        float degInRad = i*THETA;
...
void drawCylinder()
{
    ...
    float y_1 = 0.0;
    float y_2 = 0.0;
    for (float i = 0; i < 360; i=i+15.0)
    {
        x_1 = cos(i*THETA);
        x_2 = cos((i+15.0)*THETA);
        y_1 = sin(i*THETA);
        y_2 = sin((i+15.0)*THETA);
		// draw polygon 
...
void getRenderStyle()
{
    ...
    switch (renderStyle)
    {
        ...
        case MATTE:
        {
            GLfloat mat_ambient_matte[] = { 0.6, 0.48, 0.16, 1.0 };
            GLfloat mat_specular_matte[] = { 0.085, 0.064, 0.02, 1.0 };
            GLfloat low_shininess_matte[] = { 1.0 };
            GLfloat mat_emission_matte[] = {0.16, 0.12, 0.04, 0.0};
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, mat_ambient_matte);
            glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, mat_specular_matte);
            glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, low_shininess_matte);
            glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, mat_emission_matte);
            glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
            drawCube();
        }
            break;
        case METALLIC:
        {
            GLfloat mat_ambient_metal[] = { 0.6, 0.5, 0.16, 1.0 };
            GLfloat mat_specular_metal[] = { 0.85, 0.64, 0.2, 1.0 };
            GLfloat low_shininess_metal[] = { 15.0 };
            GLfloat mat_emission_metal[] = {0.16, 0.12, 0.04, 0.0};
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, mat_ambient_metal);
            glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, mat_specular_metal);
            glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, low_shininess_metal);
            glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, mat_emission_metal);
            glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
            drawCube();
            break;
        }
